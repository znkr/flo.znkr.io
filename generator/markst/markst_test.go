...
func TestLoadErrors(t *testing.T) {
	...
	_, _, err := Load(t.Context(), "site/doc/index.mst", []byte(src), nil, nil)
	if err == nil {
...
func TestLoadMetadataErrorNamesFile(t *testing.T) {
	_, _, err := Load(t.Context(), "site/doc/index.mst", []byte("= Heading\n\nNo doc-meta here.\n"), nil, nil)
	if err == nil {
		t.Fatal("Load() = nil, want error")
	}
...
func TestLoad(t *testing.T) {
	meta, rd, err := Load(t.Context(), "site/doc/index.mst", []byte(docMeta+"= Heading\n\nSome text.\n"), nil, nil)
	if err != nil {
		t.Fatalf("Load() = %v", err)
	}
...
`), nil)
	...
	t.Run("used without import", func(t *testing.T) {
		...
		meta, _, err := Load(t.Context(), "site/doc/index.mst", []byte(src), nil, libs)
		if err != nil {
			t.Fatalf("Load() = %v", err)
		}
		if meta.Title != "Test" {
	...
	t.Run("failure names the library and the call site", func(t *testing.T) {
		...
		_, _, err := Load(t.Context(), "site/doc/index.mst", []byte(src), nil, libs)
		if err == nil {
			t.Fatal("Load() = nil, want error")
		}
